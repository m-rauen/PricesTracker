#TODO: organize all the final paths in this page

#Expedia
EXPEDIA_DEPARTURE_BUTTON = "//*[@id='wizard-flight-tab-roundtrip']/div[2]/div[1]/div/div[1]/div/div/div[2]/div[1]/button"

EXPEDIA_ARRIVAL_BUTTON = "//*[@id='wizard-flight-tab-roundtrip']/div[2]/div[1]/div/div[2]/div/div/div[2]/div[1]/button"

EXPEDIA_DEPARTURE_DATE_BUTTON = "//*[@id='d1-btn']"

EXPEDIA_CONFIRM_DATE_BUTTON = "//button[@data-stid='apply-date-picker']"

EXPEDIA_SEARCH_BUTTON = "//*[@id='wizard-flight-pwa-1']/div[3]/div/button"

EXPEDIA_DEPARTURE_PATH = '//*[@id="location-field-leg1-origin"]'

EXPEDIA_ARRIVAL_PATH = '//*[@id="location-field-leg1-destination"]'

EXPEDIA_FLIGHTTIME_RESULT = "//span[@data-test-id='departure-time']"

EXPEDIA_FLIGHTPRICE_RESULT = ""


#Decolar
#TODO: add Decolar dates path

DECOLAR_DEPARTURE_PATH = "//input[@placeholder='Insira sua cidade de origem']"

DECOLAR_ARRIVAL_PATH = "//input[@placeholder='Insira sua cidade de destino']"

DECOLAR_DEPARTURE_DATE = ''

DECOLAR_ARRIVAL_DATE = ''
